...
public interface Queries {
	...
	String findAllBatchUploadsByOrderByDateUploadedDesc = "SELECT * FROM (SELECT *, (@row_number:=@row_number + 1) AS num FROM BatchUpload, (SELECT @row_number := 0) as r ORDER BY DateUploaded desc) AS RowConstrainedResult WHERE num >= ? AND num < ? ORDER BY num asc";
	String findCountBatchUpload = "SELECT COUNT(*) FROM BatchUpload";
	String findBatchUploadsByDateUploadedAfterOrderByDateUploadedDesc = "SELECT * FROM (SELECT *, (@row_number:=@row_number + 1) AS num FROM BatchUpload, (SELECT @row_number := 0) as r WHERE DateUploaded > ? ORDER BY DateUploaded desc) AS RowConstrainedResult WHERE num >= ? AND num < ? ORDER BY num asc";
	...
	**/
	...
	String findPaymentProcessorRuleBy_ID = "SELECT PaymentProcessorRuleID, PaymentProcessorID, CardType, MaximumMonthlyAmount, NoMaximumMonthlyAmountFlag, Priority, MonthToDateCumulativeAmount, CurrentYear, CurrentMonth, DateCreated, ModifiedBy FROM PaymentProcessor_Rule WHERE PaymentProcessorRuleID = ?";

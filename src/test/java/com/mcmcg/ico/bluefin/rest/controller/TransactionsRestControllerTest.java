...
public class TransactionsRestControllerTest {
	...
	public void getTransactionsOK() throws Exception { // 200
		/*
		 * List<SaleTransaction> transactions = new
		 * ArrayList<SaleTransaction>(); transactions.add(getSaleTransaction());
		 * 
		 * Mockito.when(transactionService.getLegalEntitiesFromUser(Mockito.
		 * anyString())) .thenReturn(createValidLegalEntities());
		 * Mockito.when(transactionService.getTransactions(Mockito.anyObject(),
		 * Mockito.anyInt(), Mockito.anyInt(),
		 * Mockito.anyString())).thenReturn(transactions);
		 * 
		 * mockMvc.perform(get("/api/transactions").principal(auth).param(
		 * "search", "accountNumber:67326509,amount > 4592") .param("page",
		 * "1").param("size", "2")).andExpect(status().isOk())
		 * .andExpect(content().contentType(MediaType.APPLICATION_JSON))
		 * .andExpect(jsonPath("$[0].accountNumber").value("67326509"))
		 * .andExpect(jsonPath("$[0].amount").value(new BigDecimal(4592.36)))
		 * .andExpect(jsonPath("$[0].cardNumberLast4Char").value(
		 * "XXXX-XXXX-XXXX-5162"))
		 * .andExpect(jsonPath("$[0].createdDate").value(
		 * "2016-06-07T18:05:56.555Z"))
		 ...
		 * .andExpect(jsonPath("$[0].applicationTransactionId").value(
		 * "532673163"))
		 ...
		 * Mockito.times(1)).getLegalEntitiesFromUser(Mockito.anyString());
		 * Mockito.verify(transactionService,
		 * Mockito.times(1)).getTransactions(Mockito.anyObject(),
	...
	public void getTransactionsNoSearchParam() throws Exception { // 200
		 ...
		 * 
		 * mockMvc.perform(
		 * get("/api/transactions").principal(auth).param("search",
	...
	/*
	 * @Test public void getTransactionsUnauthorizedLegalEntities() throws
	 * Exception { // 401
	 * Mockito.when(transactionService.getLegalEntitiesFromUser(Mockito.
	 * anyString())) .thenReturn(createValidLegalEntities());
	 * mockMvc.perform(get("/api/transactions").principal(auth) .param("search",
	 ...
	 * "2")).andExpect(status().isUnauthorized());
	 * 
	 * Mockito.verify(transactionService,
	 * Mockito.times(1)).getLegalEntitiesFromUser(Mockito.anyString());
	 * Mockito.verifyNoMoreInteractions(transactionService); }

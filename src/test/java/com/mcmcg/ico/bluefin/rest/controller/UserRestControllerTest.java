...
public class UserRestControllerTest {
    ...
    public void getUserAccountUnauthorizedNoObjectSupplied() throws Exception { // 401
        ...
        Mockito.verify(userService, Mockito.times(0)).belongsToSameLegalEntity(Mockito.any(Authentication.class),
                Mockito.anyString());
    ...
    @Test
    public void updateUserOKMe() throws Exception { // 200
        UpdateUserResource user = createValidUpdateResource();
        UserResource updatedUser = createValidUserResource();
        Mockito.when(userService.belongsToSameLegalEntity(Mockito.any(Authentication.class), Mockito.eq("omonge")))
                .thenReturn(true);
        Mockito.when(userService.updateUserProfile("omonge", user)).thenReturn(updatedUser);

        mockMvc.perform(put("/api/users/{username}", "me").header("X-Auth-Token", "tokenTest")
                .contentType(MediaType.APPLICATION_JSON).principal(auth).content(convertObjectToJsonBytes(user)))
        ...
        Mockito.verify(userService, Mockito.times(1)).belongsToSameLegalEntity(Mockito.any(Authentication.class),
                Mockito.eq("omonge"));
        Mockito.verify(userService, Mockito.times(1)).updateUserProfile("omonge", user);
        Mockito.verifyNoMoreInteractions(userService);
    }
    ...
    public void updateUserLegalEntitiesOKMe() throws Exception { // 200
        ...
        mockMvc.perform(
                put("/api/users/{username}/legal-entities", "me").principal(auth).header("X-Auth-Token", "tokenTest")
                        .contentType(MediaType.APPLICATION_JSON).content(convertObjectToJsonBytes(legalEntities)))
    ...
    public void updateUserLegalEntitiesUnauthorized() throws Exception { // 401
        ...
        Mockito.verify(userService, Mockito.times(0)).updateUserLegalEntities("test", legalEntities);
        Mockito.verifyNoMoreInteractions(userService);
    }
